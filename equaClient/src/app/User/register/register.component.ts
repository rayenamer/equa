...
export class RegisterComponent {
  ...
  async onSubmit() {
    ...
    if (this.recaptchaEnabled) {
      try {
        this.user.recaptchaToken = await this.executeRecaptcha('signup');
      } catch {
        this.loading = false;
  ...
  signupWithGoogle() {
    this.loading = true;
    this.errorMessage = '';
    this.authService.getGoogleLoginUrl(this.user.userType as 'OBSERVER' | 'ASSISTANT').subscribe({
      next: (googleUrl) => {
        window.location.href = googleUrl;
      },
      error: () => {
        this.loading = false;
        this.errorMessage = 'Impossible de démarrer l’inscription Google.';
      }
    });
  }
  ...
  private handleGoogleCallback() {
    ...
    if (oauthError) {
      this.errorMessage = 'L’inscription Google a échoué. Veuillez réessayer.';
      return;
    }
    ...
    this.authService.signinWithGoogleToken(token).subscribe({
      ...
      error: () => {
        this.loading = false;
        this.errorMessage = 'Inscription Google réussie, mais récupération du profil impossible.';
      }
    });
  }
